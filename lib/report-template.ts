...
function notchedFrame(className = '', aspectRatio = 3) {
  // Cross-tier geometry contract: aspect-aware SVG, inset stroke, circular
  // quarter-notch construction. Quick keeps its approved 12-unit ornament,
  // while the inset prevents Chromium from clipping any side of the frame.
  const h = 100;
...
function rankedScores(scores: AuditAnalysis['scores']) {
  return scoreOrder
    .map(key => ({ key, label: scoreLabels[key], value: scores[key] }))
    .sort((a, b) => a.value - b.value);
}

function coverSupport(analysis: AuditAnalysis) {
  const ranked = rankedScores(analysis.scores);
  const weakest = ranked[0];
  const secondWeakest = ranked[1];
  const strongest = ranked[ranked.length - 1];
  if (!weakest || !secondWeakest || !strongest) return 'The roadmap ahead prioritizes the changes most likely to create meaningful momentum first.';
  const spread = strongest.value - weakest.value;
  if (spread <= 5) {
    return `Your scores are closely grouped, with ${strongest.label.toLowerCase()} leading slightly. The roadmap focuses on the highest-value refinements across the site rather than treating any one area as a major weakness.`;
  }
  if (weakest.value >= 75) {
    return `${strongest.label} is currently your strongest area, and even ${weakest.label.toLowerCase()} - your lowest-scoring category - remains in a solid range. The roadmap is therefore about refinement rather than repair.`;
  }
  return `The clearest opportunities are ${weakest.label.toLowerCase()} and ${secondWeakest.label.toLowerCase()}, while ${strongest.label.toLowerCase()} is currently your strongest area. The roadmap prioritizes those weaker signals without losing what is already working.`;
}

function scoreMixInsight(analysis: AuditAnalysis) {
  const ranked = rankedScores(analysis.scores);
  const weakest = ranked[0];
  const strongest = ranked[ranked.length - 1];
  if (!weakest || !strongest) return 'The roadmap below turns these scores into a practical order of operations.';
  const spread = strongest.value - weakest.value;
  if (spread <= 5) {
    return `Your scores are tightly grouped, with ${strongest.label.toLowerCase()} leading at ${strongest.value}. ${weakest.label} is only slightly behind at ${weakest.value}, so the best gains are likely to come from coordinated refinements rather than one major repair.`;
  }
  if (weakest.value >= 75) {
    return `${strongest.label} is your strongest area at ${strongest.value}. Even ${weakest.label.toLowerCase()}, your lowest score at ${weakest.value}, remains solid - so the focus is on sharpening an already healthy foundation.`;
  }
...
export function renderReportHtml(input: { analysis: AuditAnalysis; site: SiteScrape; pageSpeed: PageSpeedSummary[]; tier: AuditTier; createdAt: string; competitors?: Array<{ url: string; site: SiteScrape; pageSpeed: PageSpeedSummary[] }> }) {
  ...
  pages.push(`<section class="report-page cover-page">
    ...
    <div class="executive-box">${notchedFrame('executive-frame', 3.0)}
      ...
      <p class="support-copy">${e(coverSupport(analysis))}</p>
  ...
  pages.push(`<section class="report-page glance-page">
    ...
    <div class="support-strip"><span class="support-icon">${sparkle('support-spark')}</span><p>${e(scoreMixInsight(analysis))}</p></div>
    ${pageFooter(domain, preparedDate, currentPage, totalPages)}
  </section>`);
    ...
    const actionHtml = group.map((a, idx) => {
      ...
      return `<article class="roadmap-item">
        ...
        <div class="roadmap-content">
          <div class="roadmap-top"><span class="roadmap-status ${status.cls}">${sparkle('status-spark')}${e(status.label)}</span></div>
          <h3>${e(a.title)}</h3>
          ...
          <div class="roadmap-meta"><span>Effort: ${e(a.effort)}</span>${a.pageUrl ? `<span>Page: ${e(displayPage(a.pageUrl))}</span>` : ''}</div>
        </div>
  ...
  rewriteGroups.forEach((r, idx) => {
    ...
    pages.push(`<section class="report-page rewrite-page">
      ...
      <div class="rewrite-scope">Recommendations for: <strong>${e(displayPage(r.pageUrl))}</strong></div>
  ...
  pages.push(`<section class="report-page technical-page">
    ...
    <div class="signal-grid">
      ...
      <div><strong>${site.llmsTxt ? 'Found' : 'Not detected'}</strong><span>llms.txt<small>Optional signal</small></span></div>
    </div>
