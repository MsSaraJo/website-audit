...
Tier: ${tier}.
For quick_win, prioritize the homepage and exactly the highest-value fixes. For full_site, compare patterns across pages, include useful copy rewrites, fill pageInsights for up to 5 reviewed pages with evidence-based page scores/roles/strengths/opportunities, and fill crossPageInsights with up to 4 meaningful patterns that genuinely span multiple pages. For competitor_conquest, create a premium strategic market analysis based only on supplied competitor evidence: fill competitorGap and competitorIntelligence completely. competitorIntelligence should compare the client against each supplied competitor, use 4-6 score dimensions, create competitor profiles, identify where the client leads and where competitors lead, analyze messaging/positioning gaps, identify SEO/content opportunities and market white space, recommend 4-6 differentiated win strategies, create a practical three-phase 90-day advantage plan, and explicitly distinguish smart actions from competitor tactics the client should not chase or copy. Never invent competitor claims, rankings, traffic, backlinks, revenue, market share, or metrics that are not present in the evidence. Scores are judgmental audit scores derived from supplied site evidence, not claims about actual market share or business performance.
