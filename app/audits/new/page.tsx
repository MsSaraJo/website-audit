...
export default function NewAudit(){
 ...
 async function submit(e:FormEvent){e.preventDefault();setBusy(true);setResult(null);try{const r=await fetch('/api/audits',{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${token}`},body:JSON.stringify({url,tier,competitorUrls:competitors.split('\n').map(x=>x.trim()).filter(Boolean)})});const j=await r.json();setResult(r.ok?j:{id:'',status:'error',error:j.error||'Could not start audit.'});}catch(e){setResult({id:'',status:'error',error:e instanceof Error?e.message:'Could not start audit.'});}finally{setBusy(false)}}
 useEffect(()=>{if(!result?.id||['completed','awaiting_etsy_upload','failed'].includes(result.status))return;const t=setInterval(async()=>{const r=await fetch(`/api/audits/${result.id}`,{headers:{authorization:`Bearer ${token}`}});if(r.ok)setResult(await r.json())},3000);return()=>clearInterval(t)},[result?.id,result?.status,token]);
